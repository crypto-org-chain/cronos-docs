...
Before we start, please note that there was a "*Canis Major*" network upgrade at the block height `922,363`, which requires node operator to update their Crypto.org Chain Mainnet binary `cronosd` from `v1.*.*` to `v2.0.1`. 
