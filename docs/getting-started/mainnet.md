...
Before we start, please note that there was a "*Canis Major*" network upgrade at the block height `922,363`, which requires the node operator to update their Crypto.org Chain Mainnet binary `cronosd` from `v1.*.*` to `v2.0.1`. 
...
There are two options to install `cronosd`:
- [Directly from Github](#option-1-install-cronosd-released-binaries-from-github); or
- [Homebrew](#option-2-install-cronosd-by-homebrew)
...
- You can verify the installation by checking the version of the cronosd, the current version is `1.2.1`.
  ```bash
...
- Now, install the `cronosd` version `1.2.1` with crypto-org-chain/cronosd

  ```bash
    # install the cronosd CLI tool
    $ brew install cronosd@1.2.1
  ```
...
### Step 3-2. Upgrade the `cronosd` binary to `v2.0.1`

At the upgrade height of `922,363`, users will see the following error message on the `cronosd`: 
...
- Terminate the `cronosd`; afterwards, download the `v2.0.1` released binaries from github:
  ...
  ```
    ...
    ::: tip Remarks: 
    If you have stated `cronosd` with *systemd* service, kindly stop it by 
...
- For [homebrew](https://github.com/crypto-org-chain/homebrew-cronosd#cronosd-homebrew-tap) users, simply run 

    ```bash 
    $ brew upgrade cronosd
    ```
#### Step 3-2-2 -  Verify the version

You can verify the installation by checking the version of `cronosd`, the latest version is `2.0.1`.
...
#### Step 3-2-3 - Restart `cronosd` with version `v2.0.1`
